...
class MapEventsState extends State<MapEvents> {
  ...
  void updateEventList(LatLngBounds bounds) {
    ...
    FirebaseFirestore.instance.collection('events').get().then((query) {
      events.clear();
      for (var e in query.docs) {
        ...
        if (latitudeMin <= lat && lat <= latitudeMax && longitudeMin <= lon && lon <= longitudeMax) events.add(EventMapData.fromDocumentSnapshot(e));
      }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Container(
          ...
          Expanded(
              child: flutterMapEvent(
            //events:buildEvents(),
            events: events,
            bounds: LatLngBounds(LatLng(latitudeMin, longitudeMin), LatLng(latitudeMax, longitudeMax)),
