...
class TourListWidget extends StatelessWidget {
  TourListWidget(BuildContext context, this.userId) {
    // peupler tours
    FirebaseFirestore.instance.collection('tours').where('owner', isEqualTo: userId).get().then((QuerySnapshot q) {
      for (var t in q.docs) tours.add(Tour(t));
    }).catchError((value) {
      final snackBar = SnackBar(
        content: Text('error retriving list of tours from data-base'),
